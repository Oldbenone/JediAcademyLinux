...
//	VectorCopy( self->currentOrigin, self->s.pos.trBase );
	if ( !self->client )
	{
	AngleVectors( self->s.angles, _forward, NULL, NULL );
	}

	if ( self->fxID > 0 )
	{
		G_PlayEffect( self->fxID, self->currentOrigin, _forward );
	}
...
//---------------------------------------------------------
{
	...
	{//force sight 2+ gives perfect aim
		...
		if ( ent->NPC && ent->NPC->currentAim < 5 )
		{
			...
			AngleVectors( angs, _forward, NULL, NULL );
		}
	}

	WP_MissileTargetHint(ent, start, _forward);

	gentity_t	*missile = CreateMissile( start, _forward, BRYAR_PISTOL_VEL, 10000, ent, alt_fire );
...
//	}
	...
	WP_MissileTargetHint(ent, start, _forward);
	VectorMA( start, shotRange, _forward, end );
	...
	if ( render_impact )
	{
		if ( tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage )
		{
			...
			{//hehe
				G_Damage( traceEnt, ent, ent, _forward, tr.endpos, 3, DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc );
			}
			else
			{
				G_Damage( traceEnt, ent, ent, _forward, tr.endpos, damage, DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc );
			}
		}
		else 
		{
	...
	for ( dist = 0; dist < shotDist; dist += 64 )
	{
		...
		VectorMA( start, dist, _forward, spot );
		AddSightEvent( ent, spot, 256, AEL_DISCOVERED, 50 );
	}
	VectorMA( start, shotDist-4, _forward, spot );
	AddSightEvent( ent, spot, 256, AEL_DISCOVERED, 50 );
}
...
//---------------------------------------------------------
{
	...
	else
	{
		...
		AngleVectors( ent->client->renderInfo.eyeAngles, _forward, NULL, NULL );
		...
		if ( !hitDodged )
		{
			if ( render_impact )
			{
					...
					|| traceEnt->s.eType == ET_MOVER )
				{
					...
					if ( traceEnt && traceEnt->client && traceEnt->client->NPC_class == CLASS_GALAKMECH )
					{//hehe
						G_Damage( traceEnt, ent, ent, _forward, tr.endpos, 10, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, fullCharge ? MOD_SNIPER : MOD_DISRUPTOR, hitLoc );			
						break;
					}
					G_Damage( traceEnt, ent, ent, _forward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, fullCharge ? MOD_SNIPER : MOD_DISRUPTOR, hitLoc );
...
//---------------------------------------------------------
{
	...
	G_PlayEffect( G_EffectIndex( "disruptor/line_cap" ), muzzle, _forward );
}
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( start, _forward, BOWCASTER_VELOCITY, 10000, ent, qtrue );
...
//---------------------------------------------------------
{
	...
	{
		WP_MissileTargetHint(ent, start, _forward);
		missile = CreateMissile( start, _forward, REPEATER_ALT_VELOCITY, 10000, ent, qtrue );
	}
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( start, _forward, DEMP2_VELOCITY, 10000, ent );
...
//---------------------------------------------------------
{
	...
	WP_MissileTargetHint(ent, start, _forward);
	gentity_t *missile = CreateMissile( start, _forward, DEMP2_ALT_RANGE, 1000, ent, qtrue );
...
//---------------------------------------------------------
{
	gentity_t	*missile = CreateMissile( muzzle, _forward, FLECHETTE_MINE_VEL, 10000, ent, qtrue );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( start, _forward, vel, 10000, ent, alt_fire );
	...
	if ( alt_fire )
	{
		...
		VectorCopy( _forward, missile->movedir );
...
{//a rail-gun-like beam
	...
	if (ent->s.number >= MAX_CLIENTS)
	{
		...
		AngleVectors(angles, _forward, vright, up);
	}

	//Shove us backwards for half a second
	VectorMA( ent->client->ps.velocity, -200, _forward, ent->client->ps.velocity );
...
//	}
		...
		if ( !hitDodged )
		{
			if ( render_impact )
			{
					...
					|| traceEnt->s.eType == ET_MOVER )
				{
					...
					if ( traceEnt && traceEnt->client && traceEnt->client->NPC_class == CLASS_GALAKMECH )
					{//hehe
						G_Damage( traceEnt, ent, ent, _forward, tr.endpos, 10, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, MOD_CONC_ALT, hitLoc );
						break;
					}
					G_Damage( traceEnt, ent, ent, _forward, tr.endpos, damage, DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, MOD_CONC_ALT, hitLoc );

					//do knockback and knockdown manually
					if ( traceEnt->client )
					{//only if we hit a client
						vec3_t pushDir;
						VectorCopy( _forward, pushDir );
						if ( pushDir[2] < 0.2f )
						{
							pushDir[2] = 0.2f;
	...
	for ( dist = 0; dist < shotDist; dist += 64 )
	{
		...
		G_PlayEffect( G_EffectIndex( "concussion/alt_ring" ), spot, _forward );
	}
	//FIXME: spawn a temp ent that continuously spawns sight alerts here?  And 1 sound alert to draw their attention?
	VectorMA( start, shotDist-4, _forward, spot );
	AddSightEvent( ent, spot, 256, AEL_DISCOVERED, 50 );

	G_PlayEffect( G_EffectIndex( "concussion/altmuzzle_flash" ), muzzle, _forward );
}
...
{//a fast rocket-like projectile
	...
	gentity_t *missile = CreateMissile( start, _forward, vel, 10000, ent, qfalse );
...
{
	// Chucking a new one
	AngleVectors( self->client->ps.viewangles, _forward, vright, up );
	CalcMuzzlePoint( self, _forward, vright, up, muzzle, 0 );
	...
	gentity_t	*missile = CreateMissile( start, _forward, 300, 10000, self, qfalse );
...
//---------------------------------------------------------
{
	...
	{
		WP_DropDetPack( ent, muzzle, _forward );
...
//---------------------------------------------------------
{
	...
	if ( laserTrap )
	{
		...
		VectorScale( _forward, LT_VELOCITY, laserTrap->s.pos.trDelta );
...
//---------------------------------------------------------
{
	gentity_t	*missile = CreateMissile( muzzle, _forward, BRYAR_PISTOL_VEL, 10000, ent );
...
//---------------------------------------------------------
{
	...
	WP_MissileTargetHint(ent, muzzle, _forward);

	gentity_t	*missile = CreateMissile( muzzle, _forward, vel, 10000, ent );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( muzzle, _forward, vel, 10000, ent, qtrue );
...
//---------------------------------------------------------
{
	...
	gentity_t *missile = CreateMissile( muzzle, _forward, ATST_SIDE_MAIN_VELOCITY, 10000, ent, qfalse );
...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	VectorMA( start, STUN_BATON_RANGE, _forward, end );
...
//		G_Sound( tr_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
		...
		G_Damage( tr_ent, ent, ent, _forward, tr.endpos, STUN_BATON_DAMAGE, DAMAGE_NO_KNOCKBACK, MOD_MELEE );
	}
	...
	{
		G_Damage( tr_ent, ent, ent, _forward, tr.endpos, 999, DAMAGE_NO_KNOCKBACK, MOD_MELEE ); // smash that puppy
	}
}
...
//---------------------------------------------------------
{
	...
	VectorMA( muzzle, range, _forward, end );
	...
	if ( tr_ent && tr_ent->takedamage )
	{
		...
		G_PlayEffect( G_EffectIndex( "melee/punch_impact" ), tr.endpos, _forward );
		//G_Sound( tr_ent, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
		...
		G_Damage( tr_ent, ent, ent, _forward, tr.endpos, damage, dflags, MOD_MELEE );
	}
}
...
//---------------------------------------------------------
{
	...
	gentity_t	*missile = CreateMissile( start, _forward, TUSKEN_RIFLE_VEL, 10000, ent, qfalse );
...
//---------------------------------------------------------
void CalcMuzzlePoint( gentity_t *const ent, vec3_t _forward, vec3_t right, vec3_t up, vec3_t muzzlePoint, float lead_in ) 
//---------------------------------------------------------
{
	vec3_t		org;
	...
	case WP_BLASTER_PISTOL:
		...
		muzzlePoint[2] -= 16;
		VectorMA( muzzlePoint, 28, _forward, muzzlePoint );
	...
	case WP_BLASTER:
		...
		muzzlePoint[2] -= 1;
		if ( ent->s.number == 0 )
			VectorMA( muzzlePoint, 12, _forward, muzzlePoint ); // player, don't set this any lower otherwise the projectile will impact immediately when your back is to a wall
		else
			VectorMA( muzzlePoint, 2, _forward, muzzlePoint ); // NPC, don't set too far _forward otherwise the projectile can go through doors
	...
	case WP_SABER:
		...
		VectorMA( muzzlePoint, 8, _forward, muzzlePoint );
...
void WP_RocketLock( gentity_t *ent, float lockDist )
{
	...
	vec3_t muzzleOffPoint, muzzlePoint, _forward, right, up;

	AngleVectors( ent->client->ps.viewangles, _forward, right, up );
	...
	VectorMA(muzzlePoint, muzzleOffPoint[0], _forward, muzzlePoint);
...
{//just a straight beam
	...
	if ( render_impact )
	{
		if ( tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage )
		{
			...
			int hitLoc = G_GetHitLocFromTrace( &tr, MOD_DISRUPTOR );
			G_Damage( traceEnt, ent, ent, _forward, tr.endpos, damage, DAMAGE_EXTRA_KNOCKBACK, MOD_DISRUPTOR, hitLoc );
		}
		else 
		{
			G_PlayEffect( G_EffectIndex( "disruptor/wall_impact" ), tr.endpos, tr.plane.normal );		
...
//---------------------------------------------------------
{
	...
	if ( ent->s.weapon == WP_DISRUPTOR && alt_fire )
	{
		if ( ent->NPC )
		{
			...
			AngleVectors( ent->lastAngles, _forward, vright, up );
		}
	}
	else if ( ent->s.weapon == WP_ATST_SIDE || ent->s.weapon == WP_ATST_MAIN ) 
	{
		...
		{//an NPC with no enemy to auto-aim at
			VectorCopy( ent->client->renderInfo.muzzleDir, _forward );
		}
		else
		{//NPC, auto-aim at enemy
			...
			AngleVectors (angleToEnemy1, _forward, vright, up);
		}
	} 
	...
	else 
	{
		...
		{//use our muzzleDir, can't use viewangles or vehicle m_vOrientation because we may be animated to shoot left or right...
			...
			else
			{
				...
				{
 					VectorScale(actorRight, -1.0f, _forward);
				}
				...
				{
 					VectorCopy(actorRight, _forward);
				}
				...
				if (ent->enemy)
				{
					...
					if (DotProduct(toEnemy, _forward)>0.75f && 
						((ent->s.number==0 && !Q_irand(0,2)) ||		// the player has a 1 in 3 chance
						 (ent->s.number!=0 && !Q_irand(0,5))))		// other guys have a 1 in 6 chance
					{
						VectorCopy(toEnemy, _forward);
					}
					else
					{
						_forward[0] += Q_flrand(-0.1f, 0.1f);
						_forward[1] += Q_flrand(-0.1f, 0.1f);
						_forward[2] += Q_flrand(-0.1f, 0.1f);
					}
				}
			}
		}
	...
	if (!pVeh)
	{
		if (ent->NPC && (ent->NPC->scriptFlags&SCF_FIRE_WEAPON_NO_ANIM))
		{
		 	VectorCopy( ent->client->renderInfo.muzzlePoint, muzzle );
			VectorCopy( ent->client->renderInfo.muzzleDir, _forward );
			MakeNormalVectors(_forward, vright, up);
		}
		else
		{
			CalcMuzzlePoint ( ent, _forward, vright, up, muzzle , 0);
